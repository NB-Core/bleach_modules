...
function specialtysystem_install(){
	...
	$system=array(
		...
		'race_requirements'=>array('name'=>'race_requirements', 'type'=>'varchar(500)','default'=>''),
...
function specialtysystem_showfightnav($script,$force=false) {
	...
	addnav(array("`b--Reiatsu (%s points)--`b",specialtysystem_availableuses()));
	ksort($specs);
	foreach ($specs as $key=>$data) {
		$colour=$colours[$key];
		foreach ($data as $keyi=>$dati) {
			if (is_array($dati) && ($dati['headline']<>'')) {
				addnav_notl(" ","");
				addnav_notl("`b`0-$colour".$dati['headline']."`0-`b","");
			} else {
				$dativ=explode("|||",$dati);
		...
		if ($session['user']['specialty'] == "" ||
			...
			foreach ($choices as $key=>$data) {
					...
					if (is_array($race_req)) {
						...
						foreach ($race_req as $race) {
							...
							if (substr($race,0,1) =="!") {
								...
								$race=substr($race,1);
								if (sanitize($session['user']['race'])==$race) $available=false;
						...
						if ($found==0) $available=false;
						output_notl(implode(",",$races_temp)."`n");
				...
				if (isset($data['stat_requirements']) && $data['stat_requirements']!='') {
					...
					output("`4Minimum Stat Requirements:`n");
