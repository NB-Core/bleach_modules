...
function racesystem_dohook($hookname,$args){
	...
	switch($hookname){
		...
		case "raceminedeath":
			...
			break;
		case "changesetting":
			//taken from a core race and a setting change, standard change to a new location
			...
			foreach ($races as $race) {
				...
				if ($race['racedesc']!='') {
					if (isset($race['requirements'])) {
						...
						if (isset($race['requirements']['admin'])) {
							if (($session['user']['superuser']&SU_MEGAUSER)!=SU_MEGAUSER) {
								$failed.=translate_inline("Testing: You need to be a superuser to select this.`n");
							}
						}
		...
		case "scrylocation":
			if (is_module_active("cities")) {
				foreach ($races as $race) {
					if ($race['text']!='') $args[$race['city']]="village-race-".$race['id'];
				}				
			}
			break;
		case "moderate":
			if (is_module_active("cities")) {
				tlschema("commentary");
				foreach ($races as $race) {
					if ($race['text']!='') $args["village-race-".$race['id']]=sprintf_translate("%s", $race['city']);
				}				
				tlschema();
			}
			break;
		...
		case "villagetext":
			foreach ($races as $race) {
				if ($session['user']['location'] == $race['city'] && $race['text']){
					...
					$args['section']="village-race-".sanitize($race['id']);
				}
			}
			break;
		case "stabletext":
...
function racesystem_getraces() {
	require("modules/racesystem/races.php");
	return $races;
}
